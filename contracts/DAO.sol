...
contract DA0{
    ...
    //Investor's shares get transferred to "to" address
    function transferShare(uint amount,address to) public onlyInvestor(){
        require(numOfshares[msg.sender]>=amount,"Not don't have enough shares");
        numOfshares[msg.sender]-=amount;
        if(numOfshares[msg.sender]==0){
            isInvestor[msg.sender]=false;
        }
    ...
    function createProposal(string calldata description,uint amount,address payable recepient) public onlyManager{
        ...
        recepient,0,block.timestamp+voteTime,false);
        nextProposalId++;
    }
    ...
    function executeProposal(uint ProposalId) public onlyManager(){
        Proposal storage proposal=proposals[ProposalId];
        require(((proposal.votes*100)/totalShares)>=quorum,"Majority does not suppport");
        proposal.isExecuted=true;
        _transfer(proposal.amount,proposal.recepient);
    }

    function _transfer(uint amount,address payable recepient) private{
